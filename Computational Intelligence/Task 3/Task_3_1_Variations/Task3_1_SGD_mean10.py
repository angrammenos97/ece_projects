...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(
        128, activation='relu', 
        kernel_initializer=tf.keras.initializers.RandomNormal(mean=10.0)
    ),
    tf.keras.layers.Dense(
        256, activation='relu',
        kernel_initializer=tf.keras.initializers.RandomNormal(mean=10.0)
...
## Save resutls
mdict = {
    'SGD_mean10_loss':history.history['loss'],
    'SGD_mean10_val_loss':history.history['val_loss'],
    'SGD_mean10_accuracy':history.history['accuracy'],
    'SGD_mean10_val_accuracy':history.history['val_accuracy']
}
savemat("Task3_1_SGD_mean10.mat", appendmat=True, mdict=mdict)